...
void SVGs_init(SDL_Renderer *renderer) {
...
void SVG_renderPiece(SDL_Renderer* renderer, Uint8 piece, int pos_x, int pos_y, double cell_size) {
	if (piece == 0) return;
	SDL_Rect texture_rect;
	SDL_Surface* surface = piece_tex_data[piece-1].surface;
	SDL_Texture* texture = piece_tex_data[piece-1].texture;
	texture_rect.x = pos_x + (int)(cell_size * 0.05);
	texture_rect.y = pos_y + (int)(cell_size * 0.05);
	texture_rect.w = (int)((double)surface->w / max(surface->w, surface->h) * cell_size * 0.9);
	texture_rect.h = (int)((double)surface->h / max(surface->w, surface->h) * cell_size * 0.9);
